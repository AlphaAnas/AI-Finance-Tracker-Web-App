...
import { auth, db } from 'src/app/firebase'; // Firebase imports
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      if (user) {
        ...
        // Uncomment below to redirect to login page if the user is not logged in
        // router.push('/login');  // Commented out for testing purposes
        ...
        <div className="bg-white p-6 rounded-lg shadow-md grid md:grid-cols-3 gap-6">
          ...
          <section className="md:col-span-1 border-r p-4">
            ...
            <div className="mt-2">
              ...
              <p className="bg-blue-100 p-2 rounded-md text-blue-600">{userInfo.fullName}</p>
            </div>
            <div className="mt-2">
              <p className="text-gray-500">Email</p>
              <p className="bg-blue-100 p-2 rounded-md text-blue-600">{userInfo.email}</p>
            </div>
            <div className="mt-2">
              <p className="text-gray-500">Phone No.</p>
              <p className="bg-blue-100 p-2 rounded-md text-blue-600">{userInfo.phone}</p>
            </div>
            <div className="mt-2">
              <p className="text-gray-500">Your Location</p>
              <p className="bg-blue-100 p-2 rounded-md text-blue-600">{userInfo.location}</p>
            </div>
            <div className="mt-4 flex gap-4">
              <button onClick={handleEditDetails} className="bg-blue-600 text-white px-4 py-2 rounded-md flex-1">
                Edit Details
              </button>
